# IonMod - Powershell Cmdlet Glossary
<br>
...
> This command will not pull the Zone's Records when querying all DNS Zones. Only the other data properties of the `IonZone` class.
> To populate a `IonZone` object without this data, you can do one of two things:
>  
> **1. Pipe back in, and pull just a single Zone.**
> ```Powershell
> (Get-IonZone)[0] | Get-IonZone
> ``` 
>
> **2. Initialize the Records property from a class method.**
> ```Powershell
> $OneZone = (Get-IonZone)[0]
> $OneZone.InitRecords()
> ```
> Both of these options achieve the exact same end goal. Option #2 is likely faster as it only pulls records for the Zone, where option #1 pulls data on the entire Zone. Choosing one is purely a design choice.
